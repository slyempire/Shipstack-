...
const BIDashboardModal = ({ onClose, allDns, dailyData }: { onClose: () => void; allDns: DeliveryNote[]; dailyData: any[] }) => (
  <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
    <div className="bg-white dark:bg-slate-900 w-full max-w-6xl h-[90vh] rounded-[3rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col">
      <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex justify-between items-center">
        ...
        <button onClick={onClose} className="h-10 w-10 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400 hover:text-brand transition-all shadow-sm">
          <X size={20} />
        </button>
      </div>
      <div className="flex-1 overflow-y-auto p-10 bg-slate-50/50 dark:bg-slate-900/50 space-y-8">
        <div className="grid grid-cols-3 gap-6">
          {[
            { label: 'Revenue Growth', value: '+24.5%' },
            { label: 'OpEx Efficiency', value: '92.1%' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dailyData}>
                  ...
                  <XAxis dataKey="name" fontSize={10} fontWeight="bold" stroke="#64748b" />
          ...
          <div className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm">
            ...
            <div className="h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={[{ name: 'Active', value: 75 }, { name: 'Maintenance', value: 15 }, { name: 'Idle', value: 10 }]} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                    <Cell fill="#0F2A44" />
                    <Cell fill="#F97316" />
                    <Cell fill="#E2E8F0" />
                  </Pie>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-50 dark:border-slate-700">
                  {['ID', 'Client', 'Zone', 'Status', 'Priority'].map(h => (
                    <th key={h} className={`${h === 'Priority' ? 'text-right' : 'text-left'} py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                {allDns.map(dn => (
                  <tr key={dn.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all">
                    ...
                    <td className="py-4"><Badge variant={dn.status.toLowerCase() as any}>{dn.status}</Badge></td>
                    <td className="py-4 text-right"><span className={`text-[9px] font-black uppercase tracking-widest ${dn.priority === 'HIGH' ? 'text-red-500' : 'text-slate-400'}`}>{dn.priority}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="p-8 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex justify-end">
        <button onClick={onClose} className="px-8 py-4 bg-slate-900 dark:bg-brand text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-slate-800 transition-all">
          Close Dashboard
        </button>
      </div>
    </div>
...
const DrillDownModal = ({ drillDown, onClose }: { drillDown: { type: string; value: string; data: DeliveryNote[] }; onClose: () => void }) => (
  <div className="fixed inset-0 z-[110] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
    <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[2.5rem] overflow-hidden shadow-2xl animate-in slide-in-from-bottom-8 duration-300">
      <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
        ...
        <button onClick={onClose} className="h-10 w-10 bg-slate-50 dark:bg-slate-800 rounded-xl flex items-center justify-center text-slate-400 hover:text-brand transition-all">
          <X size={20} />
        </button>
      </div>
      <div className="p-8 max-h-[60vh] overflow-y-auto space-y-4">
        {drillDown.data.map(dn => (
          <div key={dn.id} className="p-6 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 flex items-center justify-between group hover:border-brand transition-all">
            <div className="flex items-center gap-4">
              <div className="h-10 w-10 bg-white dark:bg-slate-900 rounded-xl flex items-center justify-center text-slate-300 group-hover:text-brand shadow-sm transition-all">
                <Activity size={18} />
              </div>
              <div>
      ...
      <div className="p-8 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex justify-end">
        <button onClick={onClose} className="px-8 py-4 bg-slate-900 dark:bg-brand text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl">Done</button>
      </div>
    </div>
  </div>
);
